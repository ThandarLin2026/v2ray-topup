...
TEXTS = {
    ...
    "my": {
        ...
        "contact_text": "📞 *ဆက်သွယ်ရန်*\n\nTelegram: {username}\n\n�ောက်ပါခလုတ်ကို နှိပ်၍ ကျွန်ုပ်တို့နှင့် ဆက်သွယ်ပါ။",
    ...
    "th": {
        ...
        "enter_username": "👤 *ส่ง username* (A-Z a-z 0-9 _ . -)\n/cancel เพื่อยกเลิก",
...
class Database:
    ...
    async def add_client(
        self, user_id, uuid_str, email, service_name, inbound_id, total_gb, expiry_at, plan_name, plan_days, price,
    ):
        await self.execute(
            """ INSERT INTO user_clients
               ...
               VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?) """,
            (
                user_id, uuid_str, email, service_name, inbound_id,
                expiry_at.isoformat(), total_gb, plan_name, plan_days, price,
            ),
        )
...
def build_client_status_text(client, traffic, lang, online_emails=None):
    ...
    # --- Updated status emojis based on user request ---
    if not enabled or is_expired or traffic_exhausted:
        # If disabled, expired, or traffic exhausted → Offline (Red)
        ...
        if is_expired:
            status_emoji = "🟡"  # Expired → Yellow
    else:
        if online_emails is not None:
            if email_lower in online_emails:
                status_text = "Online"
                status_emoji = "🟢"   # Online → Green
            else:
                status_text = "Offline"
                status_emoji = "🔴"   # Offline → Red
        else:
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    lang = await db.get_user_lang(user_id)
    # Improved welcome message appearance
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # State machine for various admin actions
...
async def admin_edit_topups(query, lang):
    # Using query directly; context will be accessed via global later
    current = CONFIG.get("TOPUP_AMOUNTS", [30, 60, 90, 300, 500])
    await query.message.reply_text(
        f"Current top‑up amounts: {', '.join(str(a) for a in current)}\n\n"
        "Send new comma‑separated list (e.g. 30,60,100,200) or /cancel"
    )
    # We need to store state in user_data; but query doesn't have context directly, so we use context from callback handler
    # Actually callback_handler passes context, we will set it there. This function is only used from callback
    pass  # Implementation is in callback_handler and handle_edit_topup_amounts
...
# ------- Admin Statistics with Improved "List All Users" -------
...
async def admin_list_all_users(query, lang):
    """Show every user with all their configs: QR, status, GB usage, user id"""
    ...
    for user in users:
        ...
        # Send each config with its QR, status, usage
        for idx, client in enumerate(configs, start=1):
            ...
            try:
                ...
                if svc:
                    ...
                    # Also send VLESS link with copy button
                    await send_client_config_block(query.message, client, lang)
            except Exception as e:
...
# ------- Existing Admin functions (pending topups, banks, broadcast) -------
