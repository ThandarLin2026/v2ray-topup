...
- Bank QR/Logo display
- Ubuntu 20-24 compatible
"""
...
ADMIN_NOTE = 10  # new state for admin note
...
async def callback_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "menu_back":
        ...
        # Start admin note conversation
        parts = data.split("_")
        action = parts[0]  # "approve" or "cancel"
...
async def admin_note_input(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Handle admin note input or /skip"""
    text = update.message.text
...
async def bank_holder(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text("📷 Send QR code photo (or any logo):")
    return BANK_QR
...
def main():
    ...
    admin_note_conv = ConversationHandler(
        entry_points=[CallbackQueryHandler(callback_handler, pattern="^(approve_|cancel_)")],
        states={
            ADMIN_NOTE: [
    ...
    # Important: Add admin_note_conv before other handlers to catch approve/cancel callbacks
