...
# -*- coding: utf-8 -*-
"""
Telegram VLESS Bot - Alireza0 X-UI Panel Edition
API Endpoints fixed for Alireza0 Fork
"""
...
CONFIG = {
    "BOT_TOKEN": "",
    "ADMIN_ID": 0,
    "PANEL_URL": "",       # Full web UI URL (with custom path)
    "API_BASE_URL": "",    # Actual API base (to be detected)
    "PANEL_USER": "",
    "PANEL_PASS": "",
    "INBOUND_ID": 0,
    "PORT": 0,
...
class XUIClient:
    def __init__(self):
        ...
        # Alireza0 specific API path
        self.api_inbounds_path = "/xui/API/inbounds"

    def _detect_api_base(self):
        """Try login with root first, then with custom path"""
        panel_url = CONFIG["PANEL_URL"].rstrip("/")
        # Try root (no custom path) first
        root_url = "/".join(panel_url.split("/")[:3])  # http://ip:port
    ...
    def _try_login(self, base_url: str) -> bool:
        ...
        try:
            ...
            if resp.status_code == 200 and resp.json().get("success"):
                self.base_url = base_url  # set for session
                return True
        except:
            pass
        return False
    ...
    def add_client(self, inbound_id: int, email: str, uuid_str: str) -> dict:
        # Alireza0 uses /xui/API/inbounds/addClient/
...
def main():
    ...
    bank_conv = ConversationHandler(
        entry_points=[CallbackQueryHandler(bank_name, pattern="^admin_addbank$")],
        states={
