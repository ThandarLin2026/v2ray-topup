...
# -*- coding: utf-8 -*-
"""
Telegram VLESS Bot - Final Fixed Version
- API Base URL = Panel URL (with custom path)
- All features: TopUp slip, Bank, VLESS, Admin Panel
"""
...
CONFIG = {
    "BOT_TOKEN": "",
    "ADMIN_ID": 0,
    "PANEL_URL": "",       # Full URL with custom path (used as API base)
    "PANEL_USER": "",
    "PANEL_PASS": "",
    "INBOUND_ID": 0,
    "PORT": 0,
...
# ==================== X-UI Client (Full Panel URL as base) ====================
class XUIClient:
    def __init__(self):
        # Use the full panel URL (with custom path) as API base
    ...
    def _login(self):
        ...
        if resp.status_code != 200:
            raise Exception(f"Login HTTP {resp.status_code}: {resp.text}")
        result = resp.json()
...
def get_config():
    ...
    print(f"\n✅ Configuration saved. API Base URL = {CONFIG['PANEL_URL']}\n")
