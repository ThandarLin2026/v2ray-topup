#!/usr/bin/env python3
"""
Telegram VLESS Bot - Fully Fixed
...
CONFIG = {
    "BOT_TOKEN": "",
    "ADMIN_ID": 0,
    "PANEL_URL": "",       # Web UI URL (with custom path)
    "PANEL_API_URL": "",   # API base URL (without custom path)
    "PANEL_USER": "",
    "PANEL_PASS": "",
    "INBOUND_ID": 0,
    "PORT": 0,
...
# ==================== X-UI Client (Fixed API URL) ====================
class XUIClient:
    def __init__(self):
        # Use API base URL (without custom path) from config
        self.base_url = CONFIG["PANEL_API_URL"].rstrip("/")
    ...
    def _login(self):
        ...
        if not result.get("success"):
            raise Exception(f"Login error: {result.get('msg', 'Unknown error')}")
...
async def topup_amount_selected(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Show bank list before asking slip
    banks = await db.get_banks()
    if not banks:
        await query.edit_message_text("No bank accounts available. Please contact admin.")
        return ConversationHandler.END

    bank_text = "🏦 Please transfer to one of the following accounts:\n\n"
    for bank in banks:
        bank_text += f"{bank['name']}\n{bank['number']}\n{bank['holder']}\n\n"
    bank_text += f"Amount: {amount} THB\n\n📸 Then send the payment slip photo."
...
async def approve_topup(query, topup_id: int, context: ContextTypes.DEFAULT_TYPE):
...
async def cancel_topup(query, topup_id: int, context: ContextTypes.DEFAULT_TYPE):
    topup = await db.get_topup(topup_id)
    if not topup or topup["status"] != "pending":
        await query.edit_message_text("Already processed.")
        return
...
def get_config():
    ...
    panel_url = input("Enter X-UI Panel URL (with custom path if any): ").strip()
    CONFIG["PANEL_URL"] = panel_url

    # Extract API base URL (remove custom path after port)
    # Example: http://45.91.135.123:1080/fmlzWpREgBaYkaD -> http://45.91.135.123:1080
    if "://" in panel_url:
        protocol, rest = panel_url.split("://", 1)
        host_part = rest.split("/")[0]
        CONFIG["PANEL_API_URL"] = f"{protocol}://{host_part}"
    else:
        CONFIG["PANEL_API_URL"] = panel_url.split("/")[0]
    ...
    print(f"\n✅ Configuration saved.")
    print(f"   API URL will be: {CONFIG['PANEL_API_URL']}")
    print(f"   Web UI URL: {CONFIG['PANEL_URL']}\n")
