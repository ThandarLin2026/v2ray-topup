...
# add new UDP port range
iptables -t nat -A PREROUTING -i $(ip -4 route ls | grep default | grep -Po '(?<=dev )(\S+)' | head -1) -p udp --dport 20000:50000 -j DNAT --to-destination :5667
...
# verify
iptables -t nat -L PREROUTING -n -v | grep 5667
