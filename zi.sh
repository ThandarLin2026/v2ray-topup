#!/bin/bash
# ZIVPN UDP + Topup Panel v7 - Bot Menu + Bank Fix
set -e

echo "=== ZIVPN Panel Installer v7 ==="
...
@app.route('/api/banks')
def banks():
    r=db().execute("SELECT * FROM banks ORDER BY id DESC").fetchall()
...
def topup():
    ...
    if not bank: return jsonify(ok=False,msg='Bank မရွေးရသေးပါ။ Admin ကိုပြောပါ')
...
# Conversation states
BANK_NAME, BANK_ACC, BANK_NUM = range(3)

def db(): c=sqlite3.connect(DB); c.row_factory=sqlite3.Row; return c

def admin_menu():
    kb = [
        [InlineKeyboardButton("🏦 Banks List", callback_data='menu_banks')],
        [InlineKeyboardButton("➕ Add Bank", callback_data='menu_addbank')],
        [InlineKeyboardButton("🗑️ Delete Bank", callback_data='menu_delbank')],
        [InlineKeyboardButton("👥 Users", callback_data='menu_users')]
    ]
    return InlineKeyboardMarkup(kb)

async def start(u,c):
    if u.effective_user.id!=ADMIN: return await u.message.reply_text("Admin Only")
    await u.message.reply_text(f"ZIVPN Admin Panel\nHost: {HOST}", reply_markup=admin_menu())

async def menu_callback(u,c):
    q=u.callback_query; await q.answer()
    if u.effective_user.id!=ADMIN: return
    data = q.data

    if data == 'menu_banks':
        rows=db().execute("SELECT * FROM banks ORDER BY id DESC").fetchall()
        msg="🏦 Banks List:\n\n"
        if not rows: msg += "ဘဏ်မရှိသေးပါ"
        else:
            for r in rows: msg += f"ID: {r['id']}\nName: {r['name']}\nAcc: {r['account']}\nNum: {r['number']}\n---\n"
        await q.edit_message_text(msg, reply_markup=admin_menu())

    elif data == 'menu_addbank':
        await q.edit_message_text("ဘဏ်နာမည်ထည့်ပါ:\nဥပမာ: KBANK")
        return BANK_NAME

    elif data == 'menu_delbank':
        rows=db().execute("SELECT id,name FROM banks").fetchall()
        if not rows: return await q.edit_message_text("ဖျက်စရာဘဏ်မရှိ", reply_markup=admin_menu())
        kb = [[InlineKeyboardButton(f"{r['name']} (ID:{r['id']})", callback_data=f"del_{r['id']}")] for r in rows]
        kb.append([InlineKeyboardButton("⬅️ Back", callback_data='back')])
        await q.edit_message_text("ဖျက်မယ့်ဘဏ်ရွေးပါ:", reply_markup=InlineKeyboardMarkup(kb))
        ...
        await q.edit_message_text(f"ZIVPN Admin Panel\nHost: {HOST}", reply_markup=admin_menu())

    elif data.startswith('del_'):
        bid = int(data.split('_')[1])
        db().execute("DELETE FROM banks WHERE id=?",(bid,)); db().commit()
        await q.edit_message_text("ဘဏ်ဖျက်ပြီးပါပြီ ✅", reply_markup=admin_menu())

    elif data.startswith('a_') or data.startswith('c_'):
        act,tid=data.split('_'); con=db();cur=con.cursor()
        t=cur.execute("SELECT t.*,u.username FROM topups t JOIN users u ON u.id=t.user_id WHERE t.id=?",(tid,)).fetchone()
        if not t: return await q.answer("Topup မရှိ")
        if act=='a':
...
# AddBank Conversation
async def bank_name(u,c):
    c.user_data['name'] = u.message.text
    await u.message.reply_text("Account Name ထည့်ပါ:\nဥပမာ: MR JUE HTET")
    return BANK_ACC

async def bank_acc(u,c):
    c.user_data['acc'] = u.message.text
    await u.message.reply_text("Account Number ထည့်ပါ:\nဥပမာ: 123-4-56789-0")
    return BANK_NUM

async def bank_num(u,c):
    name = c.user_data['name']
    acc = c.user_data['acc']
    num = u.message.text
    db().execute("INSERT INTO banks(name,account,number) VALUES(?,?,?)",(name,acc,num)); db().commit()
    await u.message.reply_text(f"ဘဏ်ထည့်ပြီးပါပြီ ✅\n\nName: {name}\nAcc: {acc}\nNum: {num}", reply_markup=admin_menu())
    return ConversationHandler.END

async def cancel(u,c):
    await u.message.reply_text("Cancel လုပ်ပြီးပါပြီ", reply_markup=admin_menu())
    return ConversationHandler.END

def main():
    app=Application.builder().token(TOKEN).build()
    conv = ConversationHandler(
        entry_points=[CallbackQueryHandler(menu_callback, pattern='^menu_addbank$')],
        states={
            BANK_NAME: [MessageHandler(filters.TEXT & ~filters.COMMAND, bank_name)],
            BANK_ACC: [MessageHandler(filters.TEXT & ~filters.COMMAND, bank_acc)],
            BANK_NUM: [MessageHandler(filters.TEXT & ~filters.COMMAND, bank_num)],
        },
...
# --- index.html --- Bank မရှိရင် Alert ပြမယ်
...
<style> body{background:#0a0e1a;color:#e2e8f0;font-family:system-ui}.card{background:#1e293b;border:1px solid #334155}.nav-tabs.nav-link{color:#94a3b8;border:none}.nav-tabs.nav-link.active{background:#1e293b;color:#fff} svg{width:18px;height:18px;vertical-align:-3px;margin-right:6px}.topbar{display:flex;justify-content:space-between;align-items:center;margin-bottom:1rem} </style>
</head><body class="p-3"><div class="container" style="max-width:520px">
<div class="topbar"> <h4><svg viewBox="0 0 24 24" fill="currentColor"><path d="M12 2L2 7v10c0 5.55 3.84 10.74 9 12 5.16-1.26 9-6.45 9-12V7l-10-5z"/></svg>ZIVPN PANEL</h4> <div id="userinfo"></div> </div>
<div id="loginbox" class="card p-3 mb-3" style="display:none"> <h6>Login</h6> <input id="loginuser" class="form-control mb-2" placeholder="Username"> <button onclick="login()" class="btn btn-primary w-100">Login / Register</button> </div>
<ul class="nav nav-tabs" id="maintabs" style="display:none"> <li class="nav-item"><button class="nav-link active" data-bs-toggle="tab" data-bs-target="#bal"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M3 3v18h18"/><path d="M7 16l3-3 4 4 5-5"/></svg>Balance</button></li> <li class="nav-item"><button class="nav-link" data-bs-toggle="tab" data-bs-target="#top"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><rect x="2" y="5" width="20" height="14" rx="2"/><path d="M2 10h20"/></svg>TopUp</button></li> <li class="nav-item"><button class="nav-link" data-bs-toggle="tab" data-bs-target="#srv"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><rect x="2" y="2" width="20" height="8" rx="2"/><rect x="2" y="14" width="20" height="8" rx="2"/><path d="M6 6h.01M6 18h.01"/></svg>Status</button></li> </ul>
<div class="tab-content mt-3" id="maincontent" style="display:none">
<div class="tab-pane fade show active" id="bal"> <div class="card p-3"><h5>Balance: <span id="balval">0</span> THB</h5><button onclick="logout()" class="btn btn-sm btn-secondary mt-2">Logout</button></div> <div class="card p-3 mt-3"><h6>Create Account</h6> <input id="vpnuser" class="form-control mb-2" placeholder="VPN Username"> <input id="vpnpass" class="form-control mb-2" placeholder="VPN Password"> <select id="days" class="form-control mb-2"><option value="30">30 Days - 50 THB</option><option value="60">60 Days - 80 THB</option><option value="90">90 Days - 120 THB</option></select> <button onclick="create()" class="btn btn-success w-100">Create</button></div> </div>
<div class="tab-pane fade" id="top">
    <div class="card p-3">
        <div id="bank-alert" class="alert alert-warning" style="display:none">ဘဏ်မရှိသေးပါ။ Admin ကို Telegram ကနေ ဘဏ်ထည့်ခိုင်းပါ</div>
        <select id="bank" class="form-control mb-2"></select>
        <select id="amt" class="form-control mb-2"><option value="50">50 THB</option><option value="100">100 THB</option><option value="150">150 THB</option></select>
        <input type="file" id="slip" class="form-control mb-2" accept="image/*">
        <button onclick="topup()" class="btn btn-warning w-100" id="topup-btn">Submit TopUp</button>
    </div>
</div>
...
async function loadBanks(){
    ...
    if(j.length==0){
        $('bank-alert').style.display='block';
        $('bank').style.display='none';
        $('topup-btn').disabled=true;
    }else{
        $('bank-alert').style.display='none';
        $('bank').style.display='block';
        $('topup-btn').disabled=false;
        bank.innerHTML=j.map(x=>`<option value="${x.id}">${x.name} - ${x.account} (${x.number})</option>`).join('')
    }
}
async function topup(){
    if($('bank').options.length==0) return alert('ဘဏ်မရှိသေးပါ');
    const fd=new FormData();fd.append('u',CUR.username);fd.append('amount',amt.value);fd.append('bank',bank.value);fd.append('slip',slip.files[0]);
    const r=await fetch('/api/topup',{method:'POST',body:fd});const j=await r.json();alert(j.msg);
}
...
[Unit]
Description=ZIVPN Panel Web Server
After=network.target

[Service]
WorkingDirectory=/opt/zivpn-panel
ExecStart=/usr/bin/python3 /opt/zivpn-panel/web.py
Restart=always
...
[Service]
WorkingDirectory=/opt/zivpn-panel
ExecStart=/usr/bin/python3 /opt/zivpn-panel/bot.py
Restart=always
