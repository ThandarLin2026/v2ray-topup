...
@app.route('/api/banks')
def banks(): r=db().execute("SELECT * FROM banks").fetchall(); return jsonify([dict(x) for x in r])
...
# --- index.html --- မျဉ်းသစ်တကြောင်းခြားပြီးမှ cat စမယ်
...
<script> let CUR=null;const $=id=>document.getElementById(id); function setUser(u){CUR=u;localStorage.setItem('zivpn_user',u.username);$('userinfo').innerHTML=`ID:${u.id} | ${u.username} | ${u.credit} THB`;$('balval').innerText=u.credit;$('loginbox').style.display='none';$('maintabs').style.display='flex';$('maincontent').style.display='block';loadBanks();status();} function logout(){localStorage.removeItem('zivpn_user');location.reload();} async function login(){const u=$('loginuser').value.trim();if(!u)return alert('Username');const r=await fetch('/api/user?u='+u);const j=await r.json();if(j.ok)setUser(j);} async function create(){const d={user:$('vpnuser').value,pass:$('vpnpass').value,days:$('days').value};const r=await fetch('/create',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(d)});const j=await r.json();if(!j.ok)return alert(j.msg);b.innerHTML=`IP:${j.ip}<br>Host:${j.host}<br>User:${j.user}<br>Pass:${j.pwd}<br>Exp:${j.exp}`;new bootstrap.Modal(m).show();refresh();} async function loadBanks(){const r=await fetch('/api/banks');const j=await r.json();bank.innerHTML=j.map(x=>`<option value="${x.id}">${x.name} ${x.account} ${x.number}</option>`).join('')} async function topup(){const fd=new FormData();fd.append('u',CUR.username);fd.append('amount',amt.value);fd.append('bank',bank.value);fd.append('slip',slip.files[0]);const r=await fetch('/api/topup',{method:'POST',body:fd});const j=await r.json();alert(j.msg);} async function status(){const r=await fetch('/api/status');const j=await r.json();srvres.innerHTML=`<b>Host:</b> ${j.host}<br><b>IP:</b> ${j.ip}<br><b>ZIVPN:</b> ${j.zivpn}<br><b>Active:</b> ${j.active}<br><b>Uptime:</b> ${j.uptime}`} async function refresh(){const r=await fetch('/api/user?u='+CUR.username);const j=await r.json();if(j.ok)setUser(j);} window.onload=()=>{const u=localStorage.getItem('zivpn_user');if(u){fetch('/api/user?u='+u).then(r=>r.json()).then(j=>{if(j.ok)setUser(j);else $('loginbox').style.display='block';})}else $('loginbox').style.display='block';} </script></body></html>
HTML
