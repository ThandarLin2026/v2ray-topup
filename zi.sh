#!/bin/bash
# ZIVPN UDP + Topup Panel v5 - Auto Login + ID display
set -e
apt-get update -y
...
pip3 install --break-system-packages flask python-telegram-bot==21.4 requests >/dev/null
...
<div id="loginbox" class="card p-3 mb-3" style="display:none">
  ...
  <small class="text-muted">Username တစ်ခုထည့်ပြီး login လုပ်ပါ။ အသစ်ဆို auto register လုပ်မယ်။</small>
</div>

<ul class="nav nav-tabs" id="maintabs" style="display:none">
<li class="nav-item"><button class="nav-link active" data-bs-toggle="tab" data-bs-target="#bal">
<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M3 3v18h18"/><path d="M7 16l3-3 4 4 5-5"/></svg>Balance</button></li>
<li class="nav-item"><button class="nav-link" data-bs-toggle="tab" data-bs-target="#top">
<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><rect x="2" y="5" width="20" height="14" rx="2"/><path d="M2 10h20"/></svg>TopUp</button></li>
<li class="nav-item"><button class="nav-link" data-bs-toggle="tab" data-bs-target="#srv">
<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><rect x="2" y="2" width="20" height="8" rx="2"/><rect x="2" y="14" width="20" height="8" rx="2"/><path d="M6 6h.01M6 18h.01"/></svg>Status</button></li>
</ul>

<div class="tab-content mt-3" id="maincontent" style="display:none">
<div class="tab-pane fade show active" id="bal">
  <div class="card p-3">
    <h5>Balance: <span id="balval">0</span> THB</h5>
    <button onclick="logout()" class="btn btn-sm btn-secondary mt-2">Logout</button>
  </div>
...
<div class="tab-pane fade" id="top">
  ...
  <button onclick="topup()" class="btn btn-warning w-100">Submit TopUp</button></div>
</div>
...
function setUser(u){
  CURUSER=u;
  localStorage.setItem('zivpn_user',u.username);
  $('userinfo').innerHTML=`ID: ${u.id} | ${u.username} | ${u.credit} THB`;
  $('balval').innerText=u.credit;
...
function logout(){
  localStorage.removeItem('zivpn_user');
  location.reload();
}

async function login(){
  const u=$('loginuser').value.trim();
  if(!u)return alert('Username ထည့်');
  const r=await fetch('/api/user?u='+u);const j=await r.json();
  if(j.ok)setUser(j);
}

async function create(){
  const d={user:$('vpnuser').value,pass:$('vpnpass').value,days:$('days').value};
  const r=await fetch('/create',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(d)});
  const j=await r.json();
  if(!j.ok)return alert(j.msg);
  b.innerHTML=`IP:${j.ip}<br>Host:${j.host}<br>User:${j.user}<br>Pass:${j.pwd}<br>Exp:${j.exp}<br>Days:${j.left}`;
...
async function topup(){
  if(!CURUSER)return;
  const fd=new FormData();fd.append('u',CURUSER.username);fd.append('amount',amt.value);fd.append('bank',bank.value);fd.append('slip',slip.files[0]);
...
async function refreshUser(){
  if(!CURUSER)return;
  const r=await fetch('/api/user?u='+CURUSER.username);const j=await r.json();
  if(j.ok){CURUSER=j; $('userinfo').innerHTML=`ID: ${j.id} | ${j.username} | ${j.credit} THB`; $('balval').innerText=j.credit;}
}

window.onload=()=>{
  const u=localStorage.getItem('zivpn_user');
  if(u){fetch('/api/user?u='+u).then(r=>r.json()).then(j=>{if(j.ok)setUser(j);else $('loginbox').style.display='block';})}
