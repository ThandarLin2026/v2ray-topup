...
apt-get update -y
apt-get install -y nodejs npm ufw openssl curl
...
read -p "Panel Port (example: 8080 or 3000): " PANEL_PORT
read -p "Server IP (public IP shown in panel): " SERVER_IP
read -p "Zivpn config path [/etc/zivpn/config.json]: " ZIVPN_CONFIG
...
function priceFor(days) {
  return PLAN_PRICES[Number(days)] || 0;
}
...
function syncZivpnConfig() {
  try {
    ...
    const d = loadData();
    const activePasswords = d.accounts
    ...
    try {
      ...
      console.error('Cannot parse Zivpn config:', e.message);
      return;
    }
    ...
    try {
      ...
      console.error('Restart Zivpn failed:', e.message);
    }
  } catch (e) {
...
function createAccountForUser({ tgId, panelUsername, panelPassword, days }) {
  ...
  const username = String(panelUsername || '').trim();
  const password = String(panelPassword || '').trim();
...
function approveTopup(topupId) {
  ...
  if (!topup || topup.status !== 'pending') {
    return { ok: false, error: 'Topup not found or already processed.' };
  }
...
if (bot) {
  bot.start(async (ctx) => {
    const d = loadData();
    getUser(d, ctx.from.id, ctx.from.username || '');
    saveData(d);
  ...
  bot.on('text', async (ctx) => {
    ...
    if (flow.step === 'create_days') {
      ...
      if (![30, 60].includes(days)) {
        return ctx.reply('Send only 30 or 60.');
      }
      ...
      if (!result.ok) {
        return ctx.reply(`Failed: ${result.error}`);
      }
...
echo ""
echo "Check service:"
echo "systemctl status zivpn-panel.service --no-pager"
echo ""
echo "Check port:"
echo "ss -lntp | grep :${PANEL_PORT}"
EOF
chmod +x setup-zivpn-panel.sh
echo "Script ready."
