...
read -p "Panel Port (example: 3000): " PANEL_PORT
read -p "Server IP (public IP shown in panel): " SERVER_IP
read -p "Zivpn config path [/etc/zivpn/config.json]: " ZIVPN_CONFIG_PATH
ZIVPN_CONFIG_PATH="${ZIVPN_CONFIG_PATH:-/etc/zivpn/config.json}"
...
SERVER_IP=$SERVER_IP
ZIVPN_CONFIG_PATH=$ZIVPN_CONFIG_PATH
...
const APP_DIR = process.env.APP_DIR || '/opt/zivpn-panel';
...
function ensureData() {
  if (!fs.existsSync(DATA_FILE)) {
    fs.writeFileSync(DATA_FILE, JSON.stringify({ users: [], accounts: [], topups: [] }, null, 2));
  }
}
...
function getOrCreateUser(data, userId) {
  let user = data.users.find(u => String(u.userId) === String(userId));
  if (!user) {
    user = {
...
function accountBadge(state) {
  if (state === 'active') return '<span class="badge badge-green">ACTIVE</span>';
...
function syncZivpnConfig() {
  try {
    if (!fs.existsSync(ZIVPN_CONFIG_PATH)) return;
...
function expireAccounts() {
  const data = loadData();
  let changed = false;
  for (const acc of data.accounts) {
    if (acc.status === 'active' && new Date(acc.expiresAt).getTime() <= Date.now()) {
      acc.status = 'expired';
      changed = true;
    }
  }
...
function createAccount({ userId, username, password, days }) {
  const data = loadData();
  ...
  if (!cleanUsername || !cleanPassword) return { ok: false, error: 'Username and password required' };

  if (data.accounts.some(a => a.username === cleanUsername && a.status === 'active' && new Date(a.expiresAt).getTime() > Date.now())) {
    return { ok: false, error: 'Username already exists' };
  }

  if (user.credit < plan.price) {
    return { ok: false, error: `Not enough credit. Need ${plan.price} THB.` };
  }

  user.credit -= plan.price;
...
function getTopUsers() {
  ...
  for (const t of data.topups.filter(t => t.status === 'approved')) {
    totals.set(String(t.userId), (totals.get(String(t.userId)) || 0) + Number(t.amount));
  }
  const out = [...totals.entries()].map(([userId, total]) => {
    const user = data.users.find(u => String(u.userId) === String(userId)) || { userId, credit: 0 };
    return { userId, credit: user.credit || 0, total };
  });
...
function accountCard(acc) {
  ...
  const stateText = state === 'active' ? 'active' : 'offline';
  return `
    <div class="card">
      <div class="card-top">
        <div>
          <div class="small">User ID</div>
          <div class="mono">${escapeHtml(acc.userId)}</div>
        </div>
        ${accountBadge(state)}
      </div>
      <div class="grid2">
        <div><div class="small">Username</div><div class="mono">${escapeHtml(acc.username)}</div></div>
        <div><div class="small">Password</div><div class="mono">${escapeHtml(acc.password)}</div></div>
        <div><div class="small">Plan</div><div>${escapeHtml(acc.days)} days</div></div>
        <div><div class="small">Days Left</div><div>${daysLeft(acc.expiresAt)}</div></div>
        <div><div class="small">Expired Date</div><div class="mono">${escapeHtml(formatDate(acc.expiresAt))}</div></div>
        <div><div class="small">Status</div><div class="state-${stateText}">${stateText}</div></div>
      </div>
    </div>
  `;
}

function topupCard(t) {
  ...
  return `
    <div class="card">
      <div class="card-top">
        ...
        <span class="badge ${cls}">${escapeHtml(t.status)}</span>
      </div>
      <div class="grid2">
        <div><div class="small">Amount</div><div>${escapeHtml(t.amount)} THB</div></div>
        <div><div class="small">Created</div><div class="mono">${escapeHtml(formatDate(t.createdAt))}</div></div>
      </div>
    </div>
  `;
}
...
app.get('/api/state', (req, res) => {
  ...
  const topups = getUserTopups(userId);

  res.json({
    ok: true,
    user: { userId: String(userId), credit: user.credit || 0 },
    serverIp: SERVER_IP,
    hostname: HOSTNAME,
...
app.post('/api/create-account', (req, res) => {
  const { userId, username, password, days } = req.body || {};
  const result = createAccount({ userId, username, password, days });
  ...
  return res.json({
    ...
    daysLeft: daysLeft(acc.expiresAt),
...
app.post('/api/topup', upload.single('slip'), (req, res) => {
  ...
  if (!userId) return res.status(400).json({ ok: false, error: 'userId required' });
  if (![50, 100, 150].includes(amount)) return res.status(400).json({ ok: false, error: 'Invalid amount' });
  if (!req.file) return res.status(400).json({ ok: false, error: 'Slip image required' });
  ...
  const topup = {
    ...
    userId: String(userId),
    amount,
    slipPath: `/uploads/${path.basename(req.file.path)}`,
    slipFile: req.file.path,
    status: 'pending',
    createdAt: nowIso(),
    updatedAt: nowIso(),
    approvedAt: null,
  ...
  data.topups.push(topup);
  saveData(data);

  if (bot) {
    const caption = [
      '📥 New Topup Request',
      `User ID: ${topup.userId}`,
      `Amount: ${topup.amount} THB`,
      `Status: pending`,
      `Slip: ${SERVER_IP}:${PANEL_PORT}${topup.slipPath}`
    ].join('\n');
...
app.get('/api/admin/overview', (req, res) => {
  const data = loadData();
  const pending = data.topups.filter(t => t.status === 'pending').length;
  const active = data.accounts.filter(a => accountState(a) === 'active').length;
  const offline = data.accounts.filter(a => accountState(a) === 'offline').length;
  const users = data.users.length;
  res.json({ ok: true, pending, active, offline, users });
});
...
if (bot) {
  bot.start(async (ctx) => {
    if (String(ctx.from.id) !== ADMIN_CHAT_ID) {
      return ctx.reply('Admin only bot.');
    }
    await ctx.reply('Zivpn admin bot ready.', adminKeyboard());
  });

  bot.hears('Pending Topups', async (ctx) => {
    if (String(ctx.from.id) !== ADMIN_CHAT_ID) return;
    const data = loadData();
    const pending = data.topups.filter(t => t.status === 'pending').slice(0, 20);
    if (!pending.length) return ctx.reply('No pending topups.');
    for (const t of pending) {
      ...
      const kb = Markup.inlineKeyboard([
        [
          Markup.button.callback('✅ Approve', `topup:approve:${t.id}`),
          Markup.button.callback('❌ Cancel', `topup:cancel:${t.id}`)
        ]
      ]);
      if (t.slipFile && fs.existsSync(t.slipFile)) {
        await ctx.replyWithPhoto({ source: fs.createReadStream(t.slipFile) }, { caption: txt, ...kb });
      } else {
  ...
  bot.hears('Users', async (ctx) => {
    if (String(ctx.from.id) !== ADMIN_CHAT_ID) return;
    const data = loadData();
    if (!data.users.length) return ctx.reply('No users yet.');
    const lines = data.users
      .slice()
      .sort((a, b) => Number(b.credit) - Number(a.credit))
      .slice(0, 50)
      .map(u => `User ID: ${u.userId} | Balance: ${u.credit} THB`)
      .join('\n');
  ...
  bot.hears('Accounts', async (ctx) => {
    if (String(ctx.from.id) !== ADMIN_CHAT_ID) return;
    const data = loadData();
    const list = data.accounts.slice().sort((a, b) => new Date(b.createdAt) - new Date(a.createdAt)).slice(0, 30);
    if (!list.length) return ctx.reply('No accounts yet.');
  ...
  bot.hears('Top Users', async (ctx) => {
    ...
    const text = list.map((u, i) => `${i + 1}. ${u.userId} — ${u.total} THB | Balance: ${u.credit} THB`).join('\n');
    await ctx.reply(text);
  });

  bot.action(/topup:approve:(.+)/, async (ctx) => {
    if (String(ctx.from.id) !== ADMIN_CHAT_ID) return ctx.answerCbQuery('Admin only');
...
setInterval(expireAccounts, 5 * 60 * 1000);
expireAccounts();
...
echo "Service: systemctl status zivpn-panel.service --no-pager"
echo "Logs: journalctl -u zivpn-panel.service -f"
echo "Check port: ss -lntp | grep :${PANEL_PORT}"
echo "Zivpn config synced from: ${ZIVPN_CONFIG_PATH}"
