...
def init_db():
    ...
    if cur.fetchone()["c"] == 0:
        cur.execute("INSERT INTO plans (name, days, price, traffic_gb, is_active, created_at) VALUES (?, ?, ?, ?, 1, ?)",
...
def toggle_plan(plan_id: int):
    conn = get_conn()
    cur = conn.cursor()
    cur.execute("UPDATE plans SET is_active = CASE WHEN is_active=1 THEN 0 ELSE 1 END WHERE id=?", (plan_id,))
    conn.commit()
    conn.close()
...
def toggle_bank(bank_id: int):
    conn = get_conn()
    cur = conn.cursor()
    cur.execute("UPDATE banks SET is_active = CASE WHEN is_active=1 THEN 0 ELSE 1 END WHERE id=?", (bank_id,))
    conn.commit()
    conn.close()
...
def clear_state(context):
    ...
    for k in keys:
        if k.startswith("flow_") or k in {
            ...
            "bank_name", "account_name", "account_number", "bank_note", "bank_qr",
            "plan_name", "plan_days", "plan_price", "plan_traffic",
...
async def admin_manage_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if head == "bank_toggle":
        ...
        await q.edit_message_caption("Bank toggled") if q.message.photo else await q.edit_message_text("Bank toggled")
    elif head == "bank_delete":
        delete_bank(int(data[1]))
        await q.edit_message_caption("Bank deleted") if q.message.photo else await q.edit_message_text("Bank deleted")
...
async def text_router(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if context.user_data.get("flow_add_bank") == "account_number":
        ...
        await update.message.reply_text("Note ထည့်ပါ (skip ဆိုရင် Skip ရိုက်)", reply_markup=kb([[KeyboardButton("Skip")], [KeyboardButton("🔙 Back")]]))
        return

    if context.user_data.get("flow_add_bank") == "note":
        context.user_data["bank_note"] = "" if text == "Skip" else text
        context.user_data["flow_add_bank"] = "qr"
        await update.message.reply_text("QR photo ပို့ပါ (မထည့်ချင်ရင် Skip)", reply_markup=kb([[KeyboardButton("Skip")], [KeyboardButton("🔙 Back")]]))
        return
    ...
    if context.user_data.get("flow_add_plan") == "days":
        if not text.isdigit():
            await update.message.reply_text("Days က number ဖြစ်ရမယ်")
            return
    ...
    if context.user_data.get("flow_add_plan") == "traffic":
        if not text.isdigit():
            await update.message.reply_text("Traffic GB က number ဖြစ်ရမယ်")
            return
        create_plan(
    ...
    if context.user_data.get("flow_add_panel") == "panel_name":
        ...
        await update.message.reply_text("Panel base URL ထည့်ပါ\nဥပမာ: http://ip:port/custompath", reply_markup=back_menu())
        return
    ...
    if context.user_data.get("flow_add_panel") == "inbound_id":
        if not text.isdigit():
            await update.message.reply_text("Inbound ID က number ဖြစ်ရမယ်")
            return
        create_panel(
...
def main():
    ...
    app.add_handler(MessageHandler(filters.TEXT & ~filters.COMMAND, skip_router))
