...
SUPPORT_TEXT = os.getenv("SUPPORT_TEXT", "@youradmin")
BOT_NAME = os.getenv("BOT_NAME", "JueVPN Bot")
TIMEZONE_OFFSET = int(os.getenv("TIMEZONE_OFFSET", "7"))  # Thailand = UTC+7
...
def get_topup(topup_id: int) -> Optional[sqlite3.Row]:
    conn = get_conn()
    cur = conn.cursor()
...
def cancel_topup_db(topup_id: int, admin_id: int) -> Optional[sqlite3.Row]:
    ...
    cur.execute("""
        UPDATE topups
        SET status='cancelled', admin_id=?, updated_at=?
        WHERE id=?
    """, (admin_id, ts, topup_id))
    conn.commit()
    cur.execute("SELECT * FROM topups WHERE id=?", (topup_id,))
...
class XUIClient:
    """
    IMPORTANT:
    3X-UI / X-UI fork တိုင်း API endpoint မတူနိုင်ပါတယ်။
    ဒီ class ထဲက endpoints ကို မင်း panel နဲ့ကိုက်အောင် ပြင်ပါ။
    """
    ...
    def __init__(self, panel_url: str, username: str, password: str):
        ...
        self.session.verify = False  # self-signed SSL ဖြစ်ရင် needed

    def login(self) -> bool:
        # Example only. Adjust to your X-UI fork.
        login_endpoints = [
            "/login",
            "/panel/login",
        ]
        ...
        for ep in login_endpoints:
            try:
                ...
                logger.warning("XUI login failed on %s: %s", ep, e)
        return False
    ...
    def create_client(
        ...
        # Example payload structure. Adjust to your fork.
        settings = {
            "clients": [{
                "id": client_uuid,
                "email": email,
            ...
            try:
                ...
                if r.status_code == 200:
                    return {
                        ...
                        "expiry_ts": expiry_ts,
                        "traffic_bytes": total_gb,
                        "message": f"Created via {ep}",
                    }
            except Exception as e:
                logger.warning("create_client failed on %s: %s", ep, e)

        return {
            "ok": False,
            "message": "create_client failed. Adjust X-UI endpoint in XUIClient.create_client()",
        }
    ...
    def renew_client(
        ...
        for ep in candidate_endpoints:
            try:
                ...
                logger.warning("renew_client failed on %s: %s", ep, e)

        return {
            "ok": False,
            "message": "renew_client failed. Adjust X-UI endpoint in XUIClient.renew_client()",
        }
...
async def send_admins(app: Application, text: str, reply_markup=None, photo_file_id: Optional[str] = None):
    for admin_id in ADMIN_IDS:
        try:
            ...
            logger.warning("Failed to send admin message to %s: %s", admin_id, e)
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    text = (
        f"မင်္ဂလာပါ {user['full_name'] or 'User'}\n\n"
        f"{BOT_NAME} မှ ကြိုဆိုပါတယ်။\n"
        f"ငွေဖြည့်ပြီး package ဝယ်နိုင်ပါတယ်။"
    )
...
async def my_account(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    text += (
        f"\nBalance: {float(user['balance']):.2f} {DEFAULT_CURRENCY}\n"
        f"Panel ID: {user['panel_id'] or '-'}\n"
        f"Client UUID: {user['client_uuid'] or '-'}\n"
...
async def buy_plan_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if existing_panel_id:
        ...
        if not panel:
            await query.edit_message_text("User panel data မတွေ့ပါ။ Admin ကိုဆက်သွယ်ပါ။")
            return
        if panel["status"] == "disabled":
            await query.edit_message_text("သင့် panel ကို renew မလုပ်နိုင်သေးပါ။ Admin ကိုဆက်သွယ်ပါ။")
            return
    else:
        panel = choose_active_panel_for_new_user()
        if not panel:
            await query.edit_message_text("အသစ် client ဖန်တီးဖို့ active panel မရှိသေးပါ။")
            return

    if not deduct_user_balance(user["telegram_id"], price):
        await query.edit_message_text("Balance ဖြတ်မရပါ။ နောက်တစ်ကြိမ်စမ်းပါ။")
        return
    ...
    if not xui.login():
        ...
        cur.execute("UPDATE users SET balance = balance + ?, updated_at=? WHERE telegram_id=?",
                    (price, now_ts(), user["telegram_id"]))
        conn.commit()
        conn.close()
        await query.edit_message_text("Panel login failed. Admin ကိုဆက်သွယ်ပါ။")
        return
    ...
    if user["client_uuid"]:
        ...
        await query.edit_message_text(
            ...
            f"Config UUID: {user['client_uuid']}\n"
...
async def topup_amount_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not banks:
        await query.edit_message_text("Active bank မရှိသေးပါ။ Admin ကိုဆက်သွယ်ပါ။")
        return
...
async def topup_bank_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not amount:
        await query.edit_message_text("TopUp session expired. ပြန်စပါ။")
        return
    ...
    text = (
        ...
        f"Note: {bank['note'] or '-'}\n\n"
        f"ငွေလွှဲပြီး screenshot/slip ကို ပို့ပါ။"
    )
...
async def photo_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not amount or not bank_id:
        ...
        await update.message.reply_text("TopUp session expired. ပြန်စပါ။", reply_markup=user_main_keyboard())
        return
    ...
    admin_text = (
        f"📥 New TopUp Request\n\n"
        f"TopUp ID: {topup_id}\n"
        f"User: {get_display_name(update.effective_user)}\n"
...
async def approve_cancel_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not is_admin(query.from_user.id):
        await query.edit_message_caption(caption="Unauthorized") if query.message.photo else await query.edit_message_text("Unauthorized")
        return
    ...
    if action == "approvetopup":
        ...
        if not updated:
            await query.edit_message_caption(caption="Already processed / invalid") if query.message.photo else await query.edit_message_text("Already processed / invalid")
            return
...
/togglebank BANK_ID
/listbanks
/setbankqr BANK_ID   (reply to photo or send command then next photo)
...
async def list_plans(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for r in rows:
        text += (
            f"ID={r['id']} | {r['name']} | {r['days']} days | "
            f"{r['price']} {DEFAULT_CURRENCY} | {r['traffic_gb']} GB | active={r['is_active']}\n"
        )
...
async def list_banks(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for r in rows:
        text += (
            f"ID={r['id']} | {r['bank_name']} | {r['account_name']} | "
            f"{r['account_number']} | active={r['is_active']} | qr={'yes' if r['qr_file_id'] else 'no'}\n"
        )
    await update.message.reply_text(text, reply_markup=admin_main_keyboard())
...
async def set_bank_qr(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        await update.message.reply_text(
            f"Bank ID {bank_id} အတွက် QR photo ကို အခု ပို့ပါ။",
...
async def add_topup(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        cur = conn.cursor()
        cur.execute("""
            INSERT INTO topup_options (amount, is_active, created_at)
            VALUES (?, 1, ?)
        """, (amount, now_ts()))
        conn.commit()
        topup_id = cur.lastrowid
...
async def list_panels(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for r in rows:
        text += (
            f"ID={r['id']} | {r['name']} | inbound={r['inbound_id']} | "
...
async def statistics_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    text = (
        ...
        f"Total User Balances: {float(total_balance):.2f} {DEFAULT_CURRENCY}"
    )
    await update.message.reply_text(text, reply_markup=admin_main_keyboard())
