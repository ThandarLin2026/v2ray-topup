...
- NEW: Buy Plan asks username first
- NEW: Username is used as X-UI email field
- NEW: Username duplicate check in BOTH database and X-UI panel
- NEW: Account shows ALL purchased configs with real X-UI traffic per config
- NEW: Delete Config button with confirm flow
- NEW: Contact button -> @Juevpn
"""
...
class XUIClient:
    ...
    def delete_client(self, uuid_str: str) -> bool:
        candidates = [
...
async def send_client_config_block(message_obj, client: dict):
    link = generate_vless_link(client["uuid"], client.get("plan_name", "VLESS"))
    config_text = (
        "🔐 <b>VLESS CONFIG</b>\n\n"
...
async def buy_username_input(update: Update, context: ContextTypes.DEFAULT_TYPE):
    username = sanitize_username(update.message.text or "")
