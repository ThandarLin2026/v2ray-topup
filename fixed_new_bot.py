...
def ensure_config():
    ...
    print("❌ Run manually first: python3 bot_fixed.py")
    sys.exit(1)
        ...
        for url in urls:
            try:
                ...
                try:
                    ...
                    last_error = f"Invalid JSON: {resp.text[:300]}"
                    continue

                if result.get("success"):
                    return result

                last_error = result.get("msg", "Unknown add client error")
            except Exception as e:
                last_error = str(e)

        raise Exception(f"Add client failed: {last_error}")
                ...
                if isinstance(obj, list):
                    ...
                    for item in obj:
                        if isinstance(item, str):
                            ...
                            email_value = item.get("email") or item.get("user") or item.get("remark")
                            if email_value:
                                result.add(str(email_value).lower())
...
async def process_buy_plan(update, context, desired_username, service, plan, lang):
    ...
    try:
        email_value = desired_username
        uuid_str = str(uuid.uuid4())
        expiry_dt = datetime.utcnow() + timedelta(days=days)
        expiry_ms = int(expiry_dt.timestamp() * 1000)

        if xui_client.email_exists(email_value):
        ...
        link = generate_vless_link(uuid_str, email_value, service)
...
async def show_account_by_service(query, context, service_index=None):
    ...
    if not clients:
        await query.message.reply_text(get_text("no_active_plan", lang))
        return
    ...
    for s_idx, svc in enumerate(CONFIG.get("SERVICES", [])):
        ...
        for p_idx, plan in enumerate(svc.get("plans", [])):
            keyboard.append(
                [
                    InlineKeyboardButton(
                        f"✏️ {plan['name']} - {money(plan['price'])}",
...
async def show_edit_topup_prices(query, lang):
    ...
    for idx, amount in enumerate(amounts):
        keyboard.append(
            [
                InlineKeyboardButton(
                    f"✏️ TopUp {money(amount)}",
                    callback_data=f"edittopup_{idx}",
                )
            ]
        )
