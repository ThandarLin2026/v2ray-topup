...
- Fixed missing state constants
- Auto remove old config.json on first run (force reconfigure)
"""
...
TEXTS = {
    ...
    'th': {  # Thai translations (basic, can be improved)
        'buy_plan': "🛒 ซื้อแผน",
        'topup': "💰 เติมเงิน",
        'account': "👤 บัญชี",
        'balance': "💰 ยอดเงิน",
...
def ensure_config():
    # Force remove old config.json to start fresh
    if os.path.exists(CONFIG_FILE):
        os.remove(CONFIG_FILE)
        logger.info("Removed old config.json to force fresh configuration.")
    loaded = load_config()
