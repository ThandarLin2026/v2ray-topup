...
- Direct X-UI traffic sync for accurate usage display
- Welcome message on /start
- Admin broadcast notification to all users
- VLESS CONFIG section with copy button / fallback
- Buy Plan asks username first
- Username is used as X-UI email field
- Username duplicate check in BOTH database and X-UI panel
- Account shows ALL purchased configs with real X-UI traffic per config
- Delete Config button with confirm flow
- Contact button -> @Juevpn
- FIXED: Admin Panel/menu buttons work even during username input flow
- FIXED: VLESS remarks/tag now uses buyer-entered username
- FIXED: More robust X-UI delete client handling
"""
...
def get_vless_copy_keyboard(vless_link: str):
    ...
    return InlineKeyboardMarkup([
        [InlineKeyboardButton("📋 Copy Not Supported", callback_data="copy_not_supported")]
    ])
...
async def process_buy_plan_from_message(update: Update, context: ContextTypes.DEFAULT_TYPE, plan_id: int, desired_username: str):
    ...
    try:
        ...
        if HAS_COPY_TEXT_BUTTON and CopyTextButton is not None:
            ...
            await update.message.reply_text(
                config_text + "\n\n📋 Native copy button not supported on this current library/client. Long press config to copy.",
                parse_mode="HTML",
                reply_markup=copy_keyboard
            )
