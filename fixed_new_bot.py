...
def load_config():
    ...
    try:
        ...
        if "PLANS" not in merged:
            merged["PLANS"] = DEFAULT_CONFIG["PLANS"]

        if "TOPUP_AMOUNTS" not in merged:
            merged["TOPUP_AMOUNTS"] = DEFAULT_CONFIG["TOPUP_AMOUNTS"]
...
def config_is_valid():
    ...
    if not isinstance(CONFIG.get("PLANS"), list) or len(CONFIG["PLANS"]) == 0:
        return False
...
def get_config():
    ...
    CONFIG["PLANS"] = DEFAULT_CONFIG["PLANS"]
    CONFIG["TOPUP_AMOUNTS"] = DEFAULT_CONFIG["TOPUP_AMOUNTS"]
        ...
        with sqlite3.connect(DB_FILE) as conn:
            ...
            conn.execute(
                ...
                CREATE TABLE IF NOT EXISTS user_clients (
                    ...
                    plan_price INTEGER DEFAULT 30,
                    plan_days INTEGER DEFAULT 30
                )
                """
            )

            self._add_column_if_missing(conn, "user_clients", "plan_label", "TEXT DEFAULT '1 MONTH'")
            self._add_column_if_missing(conn, "user_clients", "plan_price", "INTEGER DEFAULT 30")
    ...
    async def add_client(
        ...
        await self.execute(
            ...
            INSERT INTO user_clients (
                user_id, uuid, email, service_name, inbound_id,
                expiry_at, total_gb, plan_label, plan_price, plan_days
            )
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            """,
            (
...
class XUIClient:
    ...
    def is_client_online(self, email):
        online_emails = self.get_online_emails()

        if online_emails is None:
            return None

        return email.lower() in online_emails
...
def get_plan_by_index(index):
    try:
        return CONFIG["PLANS"][int(index)]
    except Exception:
        return None


def format_plan_name(service_name, plan):
    return f"{service_name} {{ {plan['label']} }} {plan['price']} THB"


def build_client_status_text(client, traffic, online_status, lang):
    ...
    plan_label = client.get("plan_label") or "1 MONTH"
...
async def show_service_selection(update, lang):
    ...
    for idx, svc in enumerate(services):
        keyboard.append(
            [InlineKeyboardButton(svc["name"], callback_data=f"service_{idx}")]
        )
...
async def show_package_selection(query, context, service_index):
    ...
    for idx, plan in enumerate(CONFIG.get("PLANS", [])):
        label = format_plan_name(service["name"], plan)
        keyboard.append(
            [InlineKeyboardButton(label, callback_data=f"plan_{idx}")]
        )
    ...
    await context.bot.send_message(
        ...
        text=get_text("select_package", lang),
        reply_markup=InlineKeyboardMarkup(keyboard),
        parse_mode="Markdown",
    )


async def start_buy_plan_from_plan(query, context, plan_index):
    user_id = query.from_user.id
    lang = await db.get_user_lang(user_id)

    plan = get_plan_by_index(plan_index)
    ...
    if not service_name:
        await query.answer("Service expired. Please select again.", show_alert=True)
        return
...
async def handle_buy_username(update, context):
    ...
    if not service_name or not plan:
        context.user_data.clear()
    ...
    await process_buy_plan(update, context, username, service_name, plan, lang)
    context.user_data.clear()


async def process_buy_plan(update, context, desired_username, service_name, plan, lang):
    user_id = update.effective_user.id
    svc = get_service_config(service_name)
    ...
    total_gb = int(plan.get("total_gb", 150))
    plan_label = str(plan.get("label", "1 MONTH"))
...
async def show_account(update, context, lang):
    ...
    for idx, client in enumerate(clients, start=1):
        ...
        if online_emails is None:
            ...
            online_status = client["email"].lower() in online_emails
        ...
        status_text = build_client_status_text(client, traffic, online_status, lang)
...
async def callback_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data.startswith("service_"):
        service_index = data.split("_", 1)[1]
        await show_package_selection(query, context, service_index)
        return

    if data.startswith("plan_"):
        plan_index = data.split("_", 1)[1]
        await start_buy_plan_from_plan(query, context, plan_index)
        return
