...
def is_valid_xui_email_value(username):
    ...
    # Burmese: U+1000-U+109F, Thai: U+0E00-U+0E7F
    pattern = r'^[\w\u1000-\u109F\u0E00-\u0E7F_.-]{3,32}$'
...
async def admin_edit_plan_select_plan(query, context, plan_idx):
    ...
    keyboard = InlineKeyboardMarkup([
        ...
        [InlineKeyboardButton("💾 Total GB", callback_data="editplan_field_gb")],
...
async def handle_edit_plan_field(update, context, text):
    ...
    try:
        if field == "name":
            ...
            plan["price"] = int(new_value)
        elif field == "days":
            plan["days"] = int(new_value)
        elif field == "total_gb":
            plan["total_gb"] = int(new_value)
        else:
            raise ValueError
        save_config()
...
# ---------- Other admin functions (unchanged except stats) ----------
...
# ---------- Admin Stats (unchanged but with new buttons) ----------
...
async def callback_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data.startswith("editplan_field_"):
        field = data.split("_")[-1]  # name, price, days, total_gb
