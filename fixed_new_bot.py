...
Run as a service to keep alive after SSH logout:
...
async def topup_amount_selected(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await context.bot.send_message(
        chat_id=query.message.chat_id,
        text="📸 Please send the payment slip photo after transferring.",
        parse_mode="Markdown"
    )
    return TO_SLIP
...
async def bank_qr(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if update.message.photo:
        ...
        context.user_data["bank_qr_url"] = update.message.text.strip()
...
async def edit_bank_qr(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if update.message.photo:
        ...
        qr_url = update.message.text.strip()
        qr_file_id = None
