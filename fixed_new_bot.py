...
logger = logging.getLogger("v2ray_fixed_bot")
...
TEXTS = {
    "en": {
        ...
        "no_banks": "ℹ️ No bank accounts. Please contact admin to add bank.",
    ...
    "my": {
        ...
        "purchase_success": "✅ *အောင်မြင်စွာ ဝယ်ယူပြီးပါပြီ!*\n\n📦 အစီအစဉ်: ရက် 30 (150GB)\n📅 သက်တမ်းကုန်: {expiry}\n👤 Username: `{email}`\n🔧 ဝန်ဆောင်မှု: {service}",
        "vless_config": "🔐 <b>VLESS CONFIG</b>\n\n<code>{config}</code>",
        ...
        "topup_prompt": "💰 *ငွေဖြည့်မည့်ပမာဏ:* `{amount} THB`\n\n🏦 အောက်ပါဘဏ်တစ်ခုသို့လွှဲပြီး slip ပို့ပါ။",
        "bank_caption": "🏦 *{name}*\n💳 `{number}`\n👤 {holder}\n\n💵 ပမာဏ: *{amount} THB*",
        ...
        "no_banks": "ℹ️ ဘဏ်အကောင့် မရှိသေးပါ။ ကျေးဇူးပြု၍ admin ကိုဆက်သွယ်ပါ။",
    ...
    "th": {
        ...
        "copy_fallback": "\n\n📋 กดค้างเพื่อ copy",
        "copy_btn": "📋 Copy VLESS",
        "copy_not_supported_alert": "กดค้างข้อความ config เพื่อคัดลอก",
        "account_info": "👤 บัญชี\nจำนวน Config: {count}",
        ...
        "no_banks": "ไม่มีธนาคาร กรุณาติดต่อแอดมิน",
...
class Database:
    ...
    async def add_client(self, user_id, uuid_str, email, service_name, inbound_id, total_gb, expiry_at):
        await self.execute("INSERT INTO user_clients (user_id, uuid, email, service_name, inbound_id, expiry_at, total_gb) VALUES (?, ?, ?, ?, ?, ?, ?)", (user_id, uuid_str, email, service_name, inbound_id, expiry_at.isoformat(), total_gb))
...
class XUIClient:
    ...
    def get_client_traffic(self, email):
        ...
        urls = [f"{self.base_url}/xui/API/inbounds/getClientTraffics/{encoded}", f"{self.base_url}/xui/API/inbounds/getClientTraffics/{email}"]
        for url in urls:
            try:
...
def generate_vless_link(uuid_str, remark, service_config):
    ...
    link = f"vless://{uuid_str}@{address}:{port}?path={quote(path, safe='/')}&security=none&encryption=none&type=ws&host={quote(ws_host, safe='')}"
    if remark:
...
def build_client_status_text(client, traffic, lang):
    ...
    return get_text("config_status", lang, email=client["email"], expiry=expiry_str, status_emoji=status_emoji, status=status_text, service=service_name, down=format_bytes(download), up=format_bytes(upload), used=format_bytes(total_used), percent=usage_percent, uuid=client["uuid"])
...
async def show_service_selection(update, lang):
    ...
    await update.message.reply_text(get_text("select_service", lang), reply_markup=InlineKeyboardMarkup(keyboard), parse_mode="Markdown")
...
async def start_buy_plan_from_service(query, context, service_name):
    ...
    try:
        ...
        pass
    await context.bot.send_message(chat_id=query.message.chat.id, text=get_text("enter_username", lang), parse_mode="Markdown")
...
async def process_buy_plan(update, context, desired_username, service_name, lang):
    ...
    if not is_admin:
        ...
        if balance < price:
            need = price - balance
            await update.message.reply_text(get_text("insufficient_balance", lang, balance=balance, need=need), parse_mode="Markdown")
            await send_main_menu_by_message(update.message, user_id, lang)
            return
    ...
    try:
        ...
        xui_client.add_client(svc["inbound_id"], email, uuid_str, total_gb=total_bytes, expiry_time=expiry_ms)

        await db.add_client(user_id=user_id, uuid_str=uuid_str, email=email, service_name=service_name, inbound_id=svc["inbound_id"], total_gb=total_bytes, expiry_at=expiry_dt)

        link = generate_vless_link(uuid_str, email, svc)
        qr_bytes = generate_qr_bytes(link)

        caption = get_text("purchase_success", lang, expiry=expiry_dt.strftime("%d %b %Y"), email=email, service=service_name)
...
async def show_contact(update, lang):
    await update.message.reply_text(get_contact_text(lang), parse_mode="Markdown", reply_markup=get_contact_keyboard(lang))
...
async def start_topup(update, lang):
    ...
    if not banks:
        await update.message.reply_text(get_text("no_banks", lang))
        return
    amounts = [30, 60, 90]
    keyboard = [[InlineKeyboardButton(f"💵 {a} THB", callback_data=f"topup_amt_{a}")] for a in amounts]
    keyboard.append([InlineKeyboardButton(get_text("back", lang), callback_data="menu_back")])
    await update.message.reply_text(get_text("select_amount", lang), reply_markup=InlineKeyboardMarkup(keyboard), parse_mode="Markdown")
...
async def show_account(update, context, lang):
    ...
    await update.message.reply_text(get_text("account_info", lang, count=len(clients)), parse_mode="Markdown")
    ...
    for idx, client in enumerate(clients, start=1):
        ...
        await update.message.reply_text(f"{get_text('config_header', lang, idx=idx)}\n{status_text}", parse_mode="Markdown", reply_markup=get_config_action_keyboard(client["rowid"], lang))
        ...
        try:
            ...
            if svc:
                link = generate_vless_link(client["uuid"], client["email"], svc)
                qr_bytes = generate_qr_bytes(link)
                await update.message.reply_photo(photo=qr_bytes, caption=f"📱 *QR:* `{client['email']}`", parse_mode="Markdown")
        except Exception as e:
...
async def start_topup_from_callback(query, context, amount):
    ...
    await context.bot.send_message(chat_id=query.message.chat.id, text=get_text("topup_prompt", lang, amount=amount), parse_mode="Markdown")

    for bank in banks:
        caption = get_text("bank_caption", lang, name=bank["name"], number=bank["number"], holder=bank["holder"], amount=amount)
        if bank.get("qr_file_id"):
            await context.bot.send_photo(chat_id=query.message.chat.id, photo=bank["qr_file_id"], caption=caption, parse_mode="Markdown")
        elif bank.get("qr_url"):
            await context.bot.send_photo(chat_id=query.message.chat.id, photo=bank["qr_url"], caption=caption, parse_mode="Markdown")
        else:
            await context.bot.send_message(chat_id=query.message.chat.id, text=caption, parse_mode="Markdown")

    await context.bot.send_message(chat_id=query.message.chat.id, text=get_text("send_slip", lang), parse_mode="Markdown")
...
async def receive_slip(update, context):
    ...
    try:
        await context.bot.send_photo(chat_id=CONFIG["ADMIN_ID"], photo=file_id, caption=f"🔔 *New Top-up Request*\n\n👤 User: {mention}\n💵 Amount: *{amount} THB*\n🆔 Request ID: `{topup_id}`", reply_markup=keyboard, parse_mode="Markdown")
    except Exception as e:
...
async def show_pending_topups(query, context, lang):
    ...
    for req in pending:
        keyboard = InlineKeyboardMarkup([[InlineKeyboardButton("✅ Approve", callback_data=f"approve_{req['id']}"), InlineKeyboardButton("❌ Cancel", callback_data=f"cancel_{req['id']}")]])
        caption = f"🔔 *Pending Top-up*\n\n🆔 ID: `{req['id']}`\n👤 User ID: `{req['user_id']}`\n💵 Amount: *{req['amount']} THB*\n📅 Created: `{req['created_at']}`"
...
async def handle_edit_bank_qr(update, context):
    ...
    await db.update_bank(bank_id, context.user_data["edit_bank_name"], context.user_data["edit_bank_number"], context.user_data["edit_bank_holder"], qr_file_id, qr_url)
    context.user_data.clear()
