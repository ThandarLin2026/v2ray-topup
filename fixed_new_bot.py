...
REQUIRED_PTB_VERSION = "python-telegram-bot==21.6"
...
def normalize_services():
    for svc in CONFIG.get("SERVICES", []):
        ...
        for plan in svc["plans"]:
            plan.setdefault("name", f"{svc.get('name', 'Service')} {{ PLAN }}")
            plan.setdefault("days", 30)
            plan.setdefault("price", 30)
            plan.setdefault("total_gb", 150)


def normalize_topup_amounts():
    ...
    for a in amounts:
        try:
            a = int(a)
            if a > 0 and a not in clean:
                clean.append(a)
        except Exception:
            pass

    CONFIG["TOPUP_AMOUNTS"] = sorted(clean) if clean else [30, 60, 90, 300, 500]
...
def load_config():
    ...
    try:
        ...
        normalize_services()
        normalize_topup_amounts()

        if "TOPUP_AMOUNTS" not in loaded:
            save_config()
...
TEXTS = {
    "en": {
        ...
        "copy_fallback": "\n\n📋 Config စာသားကို long press နှိပ်ပြီး copy ကူးနိုင်ပါတယ်။",
        "copy_btn": "📋 Copy Help",
        "copy_not_supported_alert": "ဒီ server version မှာ direct copy မရပါ။ Config စာသားကို long press နှိပ်ပြီး copy ကူးပါ။",
        ...
        "admin_manage_prices": "💵 Manage Plan Prices",
        "admin_manage_topups": "💰 Manage TopUp Amounts",
    ...
    "my": {
        ...
        "enter_username": (
            "👤 ဒီ config အတွက် username ပို့ပါ။\n\n"
        ...
        "purchase_success": (
            "✅ *Package ဝယ်ယူမှု အောင်မြင်ပါပြီ!*\n\n"
            "📦 Package: {plan}\n"
            "💵 စျေးနှုန်း: {price}\n"
        ...
        "copy_fallback": "\n\n📋 Config စာသားကို long press နှိပ်ပြီး copy ကူးနိုင်ပါတယ်။",
        "copy_btn": "📋 Copy Help",
        "copy_not_supported_alert": "ဒီ version မှာ direct copy မရပါ။ Config စာသားကို long press နှိပ်ပြီး copy ကူးပါ။",
        ...
        "admin_broadcast": "📢 Broadcast",
        "admin_manage_prices": "💵 Plan Price ပြင်ရန်",
        "admin_manage_topups": "💰 TopUp Amount ပြင်ရန်",
...
class Database:
    ...
    async def get_all_clients(self):
        return await self.execute(
            """
            SELECT uc.rowid, uc.*, u.username AS telegram_username, u.balance
...
def get_status_from_client(client, traffic, online_emails=None):
...
def build_client_status_text(client, traffic, lang, online_emails=None):
    ...
    status = get_status_from_client(client, traffic, online_emails)
    plan_name = client.get("plan_name") or "Package"
    limit_text = format_bytes(status["total_limit_bytes"])
    ...
    return get_text(
        ...
        plan=plan_name,
        email=client["email"],
        expiry=status["expiry_str"],
        status_emoji=status["status_emoji"],
        status=status["status_text"],
        service=service_name,
        down=format_bytes(status["download"]),
        up=format_bytes(status["upload"]),
        used=format_bytes(status["total_used"]),
        limit=limit_text,
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if state == "edit_plan_days":
        await handle_edit_plan_days(update, context)
        return

    if state == "edit_plan_gb":
        await handle_edit_plan_gb(update, context)
        return
...
async def show_balance(update, lang):
    ...
    if await db.is_admin(user_id):
        await update.message.reply_text("💰 *Admin Balance:* `Unlimited`", parse_mode="Markdown")
        return
...
async def start_topup(update, lang):
    ...
    keyboard = [
        [InlineKeyboardButton(f"💵 {money(a)}", callback_data=f"topup_amt_{a}")]
        for a in amounts
    ]
...
async def show_account(update, context, lang):
    ...
    if await db.is_admin(user_id):
        await show_admin_account_dashboard(update, context, lang)
        return
    ...
    await update.message.reply_text(
        f"{get_text('account_info', lang, count=len(clients))}\n\n📡 Service တစ်ခုရွေးပါ။",
        parse_mode="Markdown",
        reply_markup=InlineKeyboardMarkup(keyboard),
    )


async def show_user_service_configs(query, context, service_index):
    user_id = query.from_user.id
    lang = await db.get_user_lang(user_id)
    ...
    for idx, client in enumerate(clients, start=1):
        ...
        try:
            ...
            logger.warning(f"QR/config failed: {e}")


async def show_admin_account_dashboard(update, context, lang):
    ...
    text = (
        ...
        f"⚪ Total Offline: *{total_offline}*\n\n"
        "အောက်က ခလုတ်တွေကိုနှိပ်ပြီး Config တွေကြည့်နိုင်ပါတယ်။"
    )

    keyboard = InlineKeyboardMarkup(
        [
            [InlineKeyboardButton("👥 Total Users / All Configs", callback_data="admin_cfg_all")],
            [
                InlineKeyboardButton("🟢 Total Online", callback_data="admin_cfg_online"),
                InlineKeyboardButton("⚪ Total Offline", callback_data="admin_cfg_offline"),
            ],
...
async def show_admin_config_list(query, context, mode):
    user_id = query.from_user.id
    lang = await db.get_user_lang(user_id)
    ...
    for client in clients:
        ...
        if mode == "online" and not status["is_online"]:
            continue
        if mode == "offline" and status["is_online"]:
            continue
    ...
    title = {
        "all": "👥 All User Configs",
        "online": "🟢 Online Configs",
        "offline": "⚪ Offline Configs",
    }.get(mode, "Configs")
    ...
    for idx, client in enumerate(filtered, start=1):
        status_text = build_client_status_text(
            client,
            client.get("_traffic", {}),
            lang,
            online_emails,
        ...
        extra = ""
        if client.get("telegram_username"):
            extra += f"Telegram Username: @{client.get('telegram_username')}\n"
        extra += f"Balance: {money(client.get('balance', 0) or 0)}\n"
...
async def show_admin_panel(update, lang):
    keyboard = [
        ...
        [InlineKeyboardButton(get_text("admin_manage_prices", lang), callback_data="admin_prices")],
        [InlineKeyboardButton(get_text("admin_manage_topups", lang), callback_data="admin_topups")],
...
async def show_admin_prices(query, lang):
    ...
    for svc_idx, svc in enumerate(CONFIG.get("SERVICES", [])):
        keyboard.append([InlineKeyboardButton(f"📡 {svc['name']}", callback_data=f"admin_price_svc_{svc_idx}")])
...
async def show_admin_service_plans(query, service_index, lang):
    ...
    for plan_idx, plan in enumerate(service.get("plans", [])):
        text = f"{plan['name']} | {money(plan['price'])} | {plan['days']} days | {plan['total_gb']} GB"
        keyboard.append([InlineKeyboardButton(text, callback_data=f"admin_edit_plan_{service_index}_{plan_idx}")])

    keyboard.append([InlineKeyboardButton(get_text("back", lang), callback_data="admin_prices")])

    await query.message.reply_text(
        f"📡 *{service['name']}*\n\nပြင်မည့် Plan ကိုရွေးပါ။",
        reply_markup=InlineKeyboardMarkup(keyboard),
        parse_mode="Markdown",
    )


async def show_admin_edit_plan_menu(query, service_index, plan_index, lang):
    service = get_service_by_index(service_index)
    plan = get_plan_by_index(service, plan_index) if service else None
    ...
    keyboard = InlineKeyboardMarkup(
        [
            [InlineKeyboardButton("💵 Edit Price", callback_data=f"admin_set_price_{service_index}_{plan_index}")],
            [InlineKeyboardButton("📅 Edit Days", callback_data=f"admin_set_days_{service_index}_{plan_index}")],
            [InlineKeyboardButton("📊 Edit GB", callback_data=f"admin_set_gb_{service_index}_{plan_index}")],
            [InlineKeyboardButton(get_text("back", lang), callback_data=f"admin_price_svc_{service_index}")],
        ]
    )
...
async def handle_edit_plan_price(update, context):
    ...
    try:
        new_price = int((update.message.text or "").strip())
        if new_price < 0:
            raise ValueError
    except Exception:
        await update.message.reply_text("❌ Price ကို number နဲ့ပဲရိုက်ပါ။ Example: 30")
        return

    service_index = context.user_data["edit_service_index"]
    plan_index = context.user_data["edit_plan_index"]

    CONFIG["SERVICES"][service_index]["plans"][plan_index]["price"] = new_price
    save_config()
    context.user_data.clear()

    await update.message.reply_text(f"✅ Plan Price updated: {money(new_price)}")
    await send_main_menu_by_message(update.message, user_id, lang)


async def handle_edit_plan_days(update, context):
    user_id = update.effective_user.id
    lang = await db.get_user_lang(user_id)
    ...
    try:
        new_days = int((update.message.text or "").strip())
        if new_days <= 0:
            raise ValueError
    except Exception:
        await update.message.reply_text("❌ Days ကို number နဲ့ပဲရိုက်ပါ။ Example: 30")
        return

    service_index = context.user_data["edit_service_index"]
    plan_index = context.user_data["edit_plan_index"]

    CONFIG["SERVICES"][service_index]["plans"][plan_index]["days"] = new_days
    save_config()
    context.user_data.clear()

    await update.message.reply_text(f"✅ Plan Days updated: {new_days} days")
    await send_main_menu_by_message(update.message, user_id, lang)


async def handle_edit_plan_gb(update, context):
    user_id = update.effective_user.id
    lang = await db.get_user_lang(user_id)
    ...
    try:
        new_gb = int((update.message.text or "").strip())
        if new_gb <= 0:
            raise ValueError
    except Exception:
        await update.message.reply_text("❌ GB ကို number နဲ့ပဲရိုက်ပါ။ Example: 150")
        return

    service_index = context.user_data["edit_service_index"]
    plan_index = context.user_data["edit_plan_index"]

    CONFIG["SERVICES"][service_index]["plans"][plan_index]["total_gb"] = new_gb
    save_config()
    context.user_data.clear()

    await update.message.reply_text(f"✅ Plan GB updated: {new_gb} GB")
    await send_main_menu_by_message(update.message, user_id, lang)
...
async def show_admin_topups(query, lang):
    ...
    for idx, amount in enumerate(amounts):
        keyboard.append(
            [
                InlineKeyboardButton(f"✏️ {money(amount)}", callback_data=f"edit_topup_{idx}"),
                InlineKeyboardButton("❌ Delete", callback_data=f"delete_topup_{idx}"),
            ]
        )

    keyboard.append([InlineKeyboardButton("➕ Add TopUp Amount", callback_data="add_topup")])
    keyboard.append([InlineKeyboardButton(get_text("back", lang), callback_data="menu_back")])

    await query.message.reply_text(
        "💰 *Manage TopUp Amounts*",
        reply_markup=InlineKeyboardMarkup(keyboard),
        parse_mode="Markdown",
    )
...
async def handle_add_topup_amount(update, context):
    ...
    try:
        ...
        await update.message.reply_text("❌ Amount ကို number နဲ့ပဲရိုက်ပါ။ Example: 100")
        return
...
async def handle_edit_topup_amount(update, context):
    ...
    idx = context.user_data["edit_topup_index"]
    amounts = CONFIG.get("TOPUP_AMOUNTS", [])

    if idx < 0 or idx >= len(amounts):
        context.user_data.clear()
        await update.message.reply_text("❌ TopUp amount not found.")
        return
    ...
    await update.message.reply_text(f"✅ TopUp Amount updated: {money(amount)}")
    await send_main_menu_by_message(update.message, user_id, lang)
...
async def callback_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data.startswith("acctsvc_"):
        service_index = int(data.split("_")[1])
        await show_user_service_configs(query, context, service_index)
        return

    if data in ["admin_cfg_all", "admin_cfg_online", "admin_cfg_offline"]:
        mode = data.replace("admin_cfg_", "")
        await show_admin_config_list(query, context, mode)
        return
    ...
    if data.startswith("admin_price_svc_"):
        service_index = int(data.split("_")[-1])
        await show_admin_service_plans(query, service_index, lang)
        return
    ...
    if data.startswith("admin_edit_plan_"):
        ...
        await show_admin_edit_plan_menu(query, service_index, plan_index, lang)
        return
    ...
    if data.startswith("admin_set_price_"):
        ...
        context.user_data["edit_service_index"] = service_index
        context.user_data["edit_plan_index"] = plan_index

        await query.message.reply_text("💵 New price ရိုက်ပါ။ Example: 30")
        return

    if data.startswith("admin_set_days_"):
        parts = data.split("_")
        service_index = int(parts[3])
        plan_index = int(parts[4])

        context.user_data.clear()
        context.user_data["state"] = "edit_plan_days"
        context.user_data["edit_service_index"] = service_index
        context.user_data["edit_plan_index"] = plan_index

        await query.message.reply_text("📅 New days ရိုက်ပါ။ Example: 30")
        return

    if data.startswith("admin_set_gb_"):
        parts = data.split("_")
        service_index = int(parts[3])
        plan_index = int(parts[4])

        context.user_data.clear()
        context.user_data["state"] = "edit_plan_gb"
        context.user_data["edit_service_index"] = service_index
        context.user_data["edit_plan_index"] = plan_index

        await query.message.reply_text("📊 New GB ရိုက်ပါ။ Example: 150")
        return
    ...
    if data == "add_topup":
        ...
        await query.message.reply_text("➕ New TopUp Amount ရိုက်ပါ။ Example: 100")
        return

    if data.startswith("edit_topup_"):
        idx = int(data.split("_")[2])
        context.user_data.clear()
        context.user_data["state"] = "edit_topup_amount"
        context.user_data["edit_topup_index"] = idx
        await query.message.reply_text("✏️ New TopUp Amount ရိုက်ပါ။ Example: 100")
        return

    if data.startswith("delete_topup_"):
        idx = int(data.split("_")[2])
        amounts = CONFIG.get("TOPUP_AMOUNTS", [])

        if 0 <= idx < len(amounts):
            deleted = amounts.pop(idx)
            CONFIG["TOPUP_AMOUNTS"] = amounts
            save_config()
            await query.message.reply_text(f"✅ Deleted TopUp Amount: {money(deleted)}")
        else:
            await query.message.reply_text("❌ TopUp amount not found.")
