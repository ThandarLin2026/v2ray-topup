...
def install_dependencies():
    ...
    try:
        ...
        subprocess.check_call([sys.executable, "-m", "pip", "install", "--upgrade", REQUIRED_PTB_VERSION])
...
DEFAULT_CONFIG = {
    ...
    "START_MESSAGE_MY": (
        "V2RAY X-UI PANEL မှာ ကြိုဆိုပါတယ်\n"
        "AIS / TRUE / Myanmar All Sim Wifi Service များ အသုံးပြုနိုင်ပါတယ်။"
    ),
...
def money(amount):
    return f"{CONFIG.get('CURRENCY_SYMBOL', '฿')}{int(amount)}"
...
def normalize_config():
    if "CURRENCY_SYMBOL" not in CONFIG or not CONFIG["CURRENCY_SYMBOL"]:
        CONFIG["CURRENCY_SYMBOL"] = "฿"
    if "TOPUP_AMOUNTS" not in CONFIG or not isinstance(CONFIG["TOPUP_AMOUNTS"], list) or not CONFIG["TOPUP_AMOUNTS"]:
        CONFIG["TOPUP_AMOUNTS"] = [30, 60, 90, 300, 500]
    CONFIG["TOPUP_AMOUNTS"] = sorted({int(x) for x in CONFIG["TOPUP_AMOUNTS"] if int(x) > 0})
    normalize_services()
...
def get_config():
    ...
    CONFIG["CURRENCY_SYMBOL"] = input("Currency Symbol [default: ฿]: ").strip() or "฿"
...
def ensure_config():
    ...
    print("❌ Run manually first: python3 bot_updated_full.py")
    sys.exit(1)
...
TEXTS = {
    "en": {
        ...
        "admin_balance": "💰 Admin balance: Unlimited",
        ...
        "admin_edit_prices": "💵 Edit Plan Prices",
        "admin_edit_topups": "💰 Edit TopUp Prices",
        ...
        "select_account_service": "📂 Select service configs",
        "admin_dashboard": "👑 Admin Account Dashboard\n\n👥 Total Users: {users}\n📦 Total Configs: {configs}\n🟢 Total Online: {online}\n⚪ Total Offline: {offline}",
        "total_users": "👥 Total Users",
        "total_online": "🟢 Total Online",
        "total_offline": "⚪ Total Offline",
        "enter_new_price": "💵 Enter new price amount only, example: 35",
        "enter_topup_amount": "💰 Enter top-up amount only, example: 100",
        "saved": "✅ Saved.",
    },
    "my": {
        ...
        "admin_balance": "💰 Admin လက်ကျန်ငွေ: Unlimited",
        ...
        "contact_btn": "📩 Contact ဖွင့်ရန်",
        "admin_add_bank": "➕ Bank ထည့်ရန်",
        "admin_pending_topups": "📋 Pending TopUps",
        "admin_manage_banks": "🏦 Bank များစီမံရန်",
        "admin_broadcast": "📢 Broadcast",
        "admin_edit_prices": "💵 Package Price ပြင်ရန်",
        "admin_edit_topups": "💰 TopUp Price ပြင်ရန်",
        ...
        "select_account_service": "📂 ကြည့်မည့် Service Config ရွေးပါ",
        "admin_dashboard": "👑 Admin Account Dashboard\n\n👥 Total Users: {users}\n📦 Total Configs: {configs}\n🟢 Total Online: {online}\n⚪ Total Offline: {offline}",
        "total_users": "👥 Total Users",
        "total_online": "🟢 Total Online",
        "total_offline": "⚪ Total Offline",
        "enter_new_price": "💵 Price အသစ်ကို ဂဏန်းပဲရိုက်ပါ။ ဥပမာ 35",
        "enter_topup_amount": "💰 TopUp amount ကို ဂဏန်းပဲရိုက်ပါ။ ဥပမာ 100",
    ...
    "th": {
        ...
        "admin_edit_prices": "แก้ราคาแพ็กเกจ",
        "admin_edit_topups": "แก้ราคาเติมเงิน",
        ...
        "select_account_service": "เลือกบริการ",
        "admin_dashboard": "👑 Admin Dashboard\n\n👥 Total Users: {users}\n📦 Total Configs: {configs}\n🟢 Total Online: {online}\n⚪ Total Offline: {offline}",
        "total_users": "👥 Total Users",
        "total_online": "🟢 Total Online",
        "total_offline": "⚪ Total Offline",
        "enter_new_price": "ใส่ราคาใหม่ เช่น 35",
        "enter_topup_amount": "ใส่ยอดเติมเงิน เช่น 100",
        "saved": "✅ บันทึกแล้ว",
    },
}
...
def get_vless_copy_keyboard(vless_link, lang):
    try:
        return InlineKeyboardMarkup([[InlineKeyboardButton(get_text("copy_btn", lang), copy_text=CopyTextButton(text=vless_link))]])
    except Exception:
        return InlineKeyboardMarkup([[InlineKeyboardButton(get_text("copy_btn", lang), callback_data="copy_not_supported")]])


def get_config_action_keyboard(row_id, lang):
    return InlineKeyboardMarkup([[InlineKeyboardButton(get_text("confirm_delete", lang), callback_data=f"delcfg_{row_id}")]])
...
def get_contact_keyboard(lang):
    ...
    return InlineKeyboardMarkup([[InlineKeyboardButton(get_text("contact_btn", lang), url=f"https://t.me/{clean}")]])
...
def is_client_online(client, online_emails):
    if online_emails is None:
        return False
    return str(client["email"]).lower() in online_emails
...
async def send_client_config_block(message_obj, client, lang):
    ...
    await message_obj.reply_text(config_text + get_text("copy_fallback", lang), parse_mode="HTML", reply_markup=keyboard)
...
async def route_main_menu_text(update, context, text):
    ...
    if text == get_text("buy_plan", lang):
        ...
        if await db.is_admin(user_id):
            await show_admin_account(update, context, lang)
        else:
...
async def show_service_selection(update, lang):
    ...
    await update.message.reply_text(get_text("select_service", lang), reply_markup=InlineKeyboardMarkup(keyboard))
...
async def show_plan_selection_from_service(query, context, service_index):
    ...
    try:
        ...
        pass
    await context.bot.send_message(chat_id=query.message.chat.id, text=get_text("select_plan", lang), reply_markup=InlineKeyboardMarkup(keyboard))
...
async def process_buy_plan(update, context, desired_username, service, plan, lang):
    ...
    if not is_admin:
        ...
        if balance < price:
            need = price - balance
            await update.message.reply_text(get_text("insufficient_balance", lang, balance=money(balance), price=money(price), need=money(need)))
            await send_main_menu_by_message(update.message, user_id, lang)
            return
    ...
    try:
        ...
        xui_client.add_client(service["inbound_id"], email, uuid_str, total_gb=total_bytes, expiry_time=expiry_ms)
        ...
        config_text = get_text("vless_config", lang, config=html.escape(link))
        keyboard = get_vless_copy_keyboard(link, lang)
        await update.message.reply_text(config_text + get_text("copy_fallback", lang), parse_mode="HTML", reply_markup=keyboard)
        await send_main_menu_by_message(update.message, user_id, lang)
    except Exception as e:
...
async def show_balance(update, lang):
    user_id = update.effective_user.id
    if await db.is_admin(user_id):
        await update.message.reply_text(get_text("admin_balance", lang))
        return
    balance = await db.get_balance(user_id)
    await update.message.reply_text(get_text("balance_text", lang, balance=money(balance)))


async def show_contact(update, lang):
    await update.message.reply_text(get_contact_text(lang), reply_markup=get_contact_keyboard(lang))
...
async def start_topup(update, lang):
    ...
    keyboard = [[InlineKeyboardButton(money(a), callback_data=f"topup_amt_{a}")] for a in amounts]
    keyboard.append([InlineKeyboardButton(get_text("back", lang), callback_data="menu_back")])
    await update.message.reply_text(get_text("select_amount", lang), reply_markup=InlineKeyboardMarkup(keyboard))
...
async def show_account(update, context, lang):
    ...
    if not clients:
        await update.message.reply_text(get_text("no_active_plan", lang))
        return

    keyboard = []
    for idx, svc in enumerate(CONFIG.get("SERVICES", [])):
        count = len([c for c in clients if c["service_name"] == svc["name"]])
        keyboard.append([InlineKeyboardButton(f"{svc['name']} ({count})", callback_data=f"accsvc_{idx}")])
    keyboard.append([InlineKeyboardButton(get_text("back", lang), callback_data="menu_back")])

    await update.message.reply_text(
        get_text("account_info", lang, count=len(clients)) + "\n\n" + get_text("select_account_service", lang),
        reply_markup=InlineKeyboardMarkup(keyboard),
    )


async def show_user_service_configs(chat_id, context, user_id, lang, service_index):
    service = get_service_by_index(service_index)
    if not service:
        await context.bot.send_message(chat_id, "Service not found.")
        return
    clients = await db.get_clients_by_service(user_id, service["name"])
    if not clients:
        await context.bot.send_message(chat_id, get_text("no_active_plan", lang))
        return

    await context.bot.send_message(chat_id, f"📂 {service['name']}\n📦 Configs: {len(clients)}")
    online_emails = xui_client.get_online_emails() if xui_client else None
    await send_clients_list(context, chat_id, clients, lang, online_emails=online_emails, include_qr=True, include_config=True)


async def send_clients_list(context, chat_id, clients, lang, online_emails=None, include_qr=False, include_config=False):
        ...
        if include_qr:
            try:
                ...
                if svc:
                    link = generate_vless_link(client["uuid"], client["email"], svc)
                    qr_bytes = generate_qr_bytes(link)
                    await context.bot.send_photo(chat_id=chat_id, photo=qr_bytes, caption=f"📱 QR: {client['email']}")
            except Exception as e:
        ...
        if include_config:
            try:
                ...
                if svc:
                    ...
                    config_text = get_text("vless_config", lang, config=html.escape(link))
                    keyboard = get_vless_copy_keyboard(link, lang)
                    await context.bot.send_message(chat_id=chat_id, text=config_text + get_text("copy_fallback", lang), parse_mode="HTML", reply_markup=keyboard)
            except Exception as e:
                logger.warning(f"Config send failed: {e}")


async def build_admin_counts():
    users = await db.get_all_users()
    clients = await db.get_all_clients()
    online_emails = xui_client.get_online_emails() if xui_client else set()
    if online_emails is None:
        online_emails = set()
    online_clients = [c for c in clients if is_client_online(c, online_emails)]
    offline_clients = [c for c in clients if not is_client_online(c, online_emails)]
    return users, clients, online_clients, offline_clients, online_emails


async def show_admin_account(update, context, lang):
    users, clients, online_clients, offline_clients, online_emails = await build_admin_counts()

    keyboard = InlineKeyboardMarkup(
        [
            [InlineKeyboardButton(get_text("total_users", lang), callback_data="admstats_users")],
            [
                InlineKeyboardButton(get_text("total_online", lang), callback_data="admstats_online"),
                InlineKeyboardButton(get_text("total_offline", lang), callback_data="admstats_offline"),
            ],
            [InlineKeyboardButton(get_text("back", lang), callback_data="menu_back")],
        ]
    )
    await update.message.reply_text(
        get_text("admin_dashboard", lang, users=len(users), configs=len(clients), online=len(online_clients), offline=len(offline_clients)),
...
async def start_topup_from_callback(query, context, amount):
    ...
    await context.bot.send_message(chat_id=query.message.chat.id, text=get_text("topup_prompt", lang, amount=money(amount)))
    for bank in banks:
        caption = get_text("bank_caption", lang, name=bank["name"], number=bank["number"], holder=bank["holder"], amount=money(amount))
        if bank.get("qr_file_id"):
            await context.bot.send_photo(chat_id=query.message.chat.id, photo=bank["qr_file_id"], caption=caption)
        elif bank.get("qr_url"):
            await context.bot.send_photo(chat_id=query.message.chat.id, photo=bank["qr_url"], caption=caption)
        else:
            await context.bot.send_message(chat_id=query.message.chat.id, text=caption)

    await context.bot.send_message(chat_id=query.message.chat.id, text=get_text("send_slip", lang))
...
async def receive_slip(update, context):
    ...
    keyboard = InlineKeyboardMarkup(
        [[InlineKeyboardButton("✅ Approve", callback_data=f"approve_{topup_id}"), InlineKeyboardButton("❌ Cancel", callback_data=f"cancel_{topup_id}")]]
    )
...
async def show_admin_panel(update, lang):
    keyboard = [
        ...
        [InlineKeyboardButton(get_text("admin_edit_prices", lang), callback_data="admin_prices")],
        [InlineKeyboardButton(get_text("admin_edit_topups", lang), callback_data="admin_topups")],
...
async def show_admin_prices(query, lang):
    keyboard = []
    for sidx, svc in enumerate(CONFIG.get("SERVICES", [])):
        keyboard.append([InlineKeyboardButton(f"📡 {svc['name']}", callback_data=f"admin_pricesvc_{sidx}")])
    keyboard.append([InlineKeyboardButton(get_text("back", lang), callback_data="menu_back")])
    await query.message.reply_text("💵 Select service to edit plan price", reply_markup=InlineKeyboardMarkup(keyboard))


async def show_admin_service_plans(query, service_index, lang):
    svc = get_service_by_index(service_index)
    if not svc:
        await query.answer("Service not found.", show_alert=True)
        return

    keyboard = []
    for pidx, plan in enumerate(svc.get("plans", [])):
        keyboard.append([InlineKeyboardButton(f"{plan['name']} - {money(plan['price'])}", callback_data=f"editplan_{service_index}_{pidx}")])
    keyboard.append([InlineKeyboardButton(get_text("back", lang), callback_data="admin_prices")])
    await query.message.reply_text(f"💵 {svc['name']}\nSelect plan to edit price", reply_markup=InlineKeyboardMarkup(keyboard))
...
async def handle_edit_plan_price(update, context):
    ...
    if not text.isdigit() or int(text) <= 0:
        await update.message.reply_text(get_text("enter_new_price", lang))
        return

    sidx = context.user_data.get("edit_service_index")
    pidx = context.user_data.get("edit_plan_index")
    svc = get_service_by_index(sidx)
    plan = get_plan_by_index(svc, pidx) if svc else None
    ...
    save_config()
    context.user_data.clear()
    await update.message.reply_text(f"{get_text('saved', lang)}\n{plan['name']} = {money(plan['price'])}")
    await send_main_menu_by_message(update.message, user_id, lang)


async def show_admin_topups(query, lang):
    amounts = CONFIG.get("TOPUP_AMOUNTS", [])
    keyboard = []
    for amount in amounts:
        keyboard.append(
            [
                InlineKeyboardButton(f"💰 {money(amount)}", callback_data="noop"),
                InlineKeyboardButton("❌ Delete", callback_data=f"deltopup_{amount}"),
            ]
        )
    keyboard.append([InlineKeyboardButton("➕ Add TopUp Amount", callback_data="addtopup")])
    keyboard.append([InlineKeyboardButton(get_text("back", lang), callback_data="menu_back")])
    await query.message.reply_text("💰 Manage TopUp Prices", reply_markup=InlineKeyboardMarkup(keyboard))
...
async def handle_add_topup_amount(update, context):
    ...
    text = (update.message.text or "").strip()
    if not text.isdigit() or int(text) <= 0:
        await update.message.reply_text(get_text("enter_topup_amount", lang))
        return

    amount = int(text)
    amounts = set(CONFIG.get("TOPUP_AMOUNTS", []))
    amounts.add(amount)
    CONFIG["TOPUP_AMOUNTS"] = sorted(amounts)
    save_config()
    context.user_data.clear()
    await update.message.reply_text(f"{get_text('saved', lang)}\nTopUp: {money(amount)}")
    await send_main_menu_by_message(update.message, user_id, lang)
...
async def show_pending_topups(query, context, lang):
    ...
    for req in pending:
        keyboard = InlineKeyboardMarkup(
            [[InlineKeyboardButton("✅ Approve", callback_data=f"approve_{req['id']}"), InlineKeyboardButton("❌ Cancel", callback_data=f"cancel_{req['id']}")]]
        )
        caption = (
            f"🔔 *Pending Top-up*\n\n"
            f"🆔 ID: `{req['id']}`\n"
...
async def manage_banks(query, lang):
    ...
    keyboard.append([InlineKeyboardButton(get_text("back", lang), callback_data="menu_back")])
    await query.message.reply_text("🏦 *Manage Banks*", reply_markup=InlineKeyboardMarkup(keyboard), parse_mode="Markdown")
...
async def handle_bank_qr(update, context):
    ...
    await db.add_bank(context.user_data["bank_name"], context.user_data["bank_number"], context.user_data["bank_holder"], qr_file_id, qr_url)
...
async def callback_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data.startswith("lang_"):
        ...
        try:
            ...
            pass
        await context.bot.send_message(query.message.chat.id, get_text("lang_changed", code))
    ...
    if data.startswith("accsvc_"):
        service_index = int(data.split("_")[1])
        await show_user_service_configs(query.message.chat.id, context, user_id, lang, service_index)
        return

    if data.startswith("admstats_"):
        if not await db.is_admin(user_id):
        ...
        if mode == "users":
            await query.message.reply_text(f"👥 Total Users: {len(users)}\n📦 Showing all user configs with User IDs.")
            selected = clients
        elif mode == "online":
            await query.message.reply_text(f"🟢 Total Online Configs: {len(online_clients)}")
            selected = online_clients
        else:
            await query.message.reply_text(f"⚪ Total Offline Configs: {len(offline_clients)}")
            selected = offline_clients

        if not selected:
            await query.message.reply_text("No configs found.")
            return

        await send_clients_list(context, query.message.chat.id, selected, lang, online_emails=online_emails, include_qr=False, include_config=False)
        return
    ...
    if data.startswith("confirmdelcfg_"):
        ...
        try:
            ...
            await query.edit_message_text(get_text("config_deleted", lang, email=client["email"], uuid=client["uuid"]))
        except Exception as e:
            logger.error(f"Delete config failed: {e}")
            await query.edit_message_text(get_text("delete_failed", lang, error=str(e)[:300]))
        return
    ...
    if data == "admin_prices":
        await show_admin_prices(query, lang)
        return

    if data.startswith("admin_pricesvc_"):
        sidx = int(data.split("_")[2])
        await show_admin_service_plans(query, sidx, lang)
        return
    ...
    if data.startswith("editplan_"):
        ...
        context.user_data["edit_service_index"] = sidx
        context.user_data["edit_plan_index"] = pidx
        await query.message.reply_text(f"{plan['name']}\nCurrent Price: {money(plan['price'])}\n\n{get_text('enter_new_price', lang)}")
        return

    if data == "admin_topups":
        await show_admin_topups(query, lang)
        return

    if data == "addtopup":
        context.user_data.clear()
        context.user_data["state"] = "add_topup_amount"
        await query.message.reply_text(get_text("enter_topup_amount", lang))
        return

    if data.startswith("deltopup_"):
        amount = int(data.split("_")[1])
        CONFIG["TOPUP_AMOUNTS"] = [x for x in CONFIG.get("TOPUP_AMOUNTS", []) if int(x) != amount]
        if not CONFIG["TOPUP_AMOUNTS"]:
            CONFIG["TOPUP_AMOUNTS"] = [30]
        save_config()
        await query.message.reply_text(f"{get_text('saved', lang)}\nDeleted TopUp: {money(amount)}")
        return
