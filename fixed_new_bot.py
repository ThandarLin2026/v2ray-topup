...
DEFAULT_CONFIG = {
    ...
    "TOPUP_AMOUNTS": [30, 60, 90, 300, 500],   # editable by admin
}
...
TEXTS = {
    ...
    "my": {
        ...
        "list_all_users": "📋 အသုံးပြုသူအားလုံး",
...
async def show_configs_for_service(query, context, service_name):
    ...
    if not clients:
        ...
        return
    # Send each config separately (like old behaviour, but filtered)
...
# ---------- Existing show_account replaced by the above, but keep old for compatibility? Not needed ----------
...
async def admin_edit_topups(query, lang):
    context = query._context  # hack to get context? Actually callback_handler passes context
...
async def admin_list_all_users(query, lang):
    ...
    for u in users:
        ...
        text = f"🆔 `{u['user_id']}` | Bal: {u['balance']} ฿ | Configs: {len(configs)}"
        keyboard = InlineKeyboardMarkup([
            [InlineKeyboardButton("➕ Add Credit", callback_data=f"admin_add_credit_to_{u['user_id']}")]
        ])
...
async def admin_list_online_configs(query, lang):
    ...
    if not online_emails:
        ...
        return
    # Find configs in DB with those emails
    all_configs = []
    users = await db.get_all_users()
    for user in users:
        configs = await db.get_clients(user["user_id"])
...
async def callback_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data.startswith("editplan_plan_"):
        parts = data.split("_")
        # data format: editplan_plan_0, we need service idx from context
        service_idx = context.user_data.get("edit_plans_service_idx")
    ...
    if data == "admin_panel":
        ...
        await show_admin_panel(query, lang)  # query.message is the message, but show_admin_panel expects update.message? We'll adapt
        # Actually show_admin_panel expects update, not query. We'll send new message
    ...
    if data == "admin_user_mgmt":
        await admin_list_all_users(query, lang)  # reuse
        return
