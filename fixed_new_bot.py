...
def install_dependencies():
    ...
    # This bot uses async Application/ContextTypes/filters and CopyTextButton.
    # They are not available in python-telegram-bot 13.x, so upgrade if an old version is installed.
    try:
        import telegram
...
TEXTS = {
    ...
    "my": {
        ...
        "select_amount": "💰 *ငွေဖြည့်မည့် ပမာဏရွေးပါ*",
        "select_plan": "📦 *အစီအစဉ်*\nရက် 30 (150GB) - 30 ဘတ်",
        "enter_username": (
        ...
        "vless_config": "🔐 <b>VLESS CONFIG</b>\n\n<code>{config}</code>",
        "copy_fallback": "\n\n📋 Copy VLESS ခလုတ်နှိပ်ပါ။ မရပါက Config စာသားကိုဖိထားပြီး copy ကူးပါ။",
        "copy_btn": "📋 Copy VLESS",
        "copy_not_supported_alert": "သင့် Telegram app မှာ direct copy မရပါ။ Config စာသားကိုဖိထားပြီး copy ကူးပါ။",
        ...
        "no_banks": "ℹ️ ဘဏ်အကောင့် မရှိသေးပါ။ Admin Panel ကနေ ဘဏ်အကောင့်အရင်ထည့်ပါ။",
...
class XUIClient:
    ...
    def _post_json_or_form(self, url, data=None, json_data=None, timeout=20):
                ...
                try:
                    ...
                    for client in settings.get("clients", []):
                        if str(client.get("email", "")).lower() == email.lower():
                            ...
                            # Some panels keep usage in clientStats, not settings.clients.
    ...
    def delete_client(self, uuid_str, inbound_id=None, email=None):
        ...
        # Last safety check: if panel no longer has the client, treat as success.
...
async def callback_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # copy_text buttons do not send callbacks. This branch is only for fallback keyboards.
